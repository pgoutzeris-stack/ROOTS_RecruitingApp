...
const RoundSwitchDialog = memo(({ onConfirm, onCancel }) => (
  <div onClick={onCancel} role="dialog" aria-modal="true" aria-label="Rundenwechsel bestätigen"
    style={{ position: 'fixed', inset: 0, background: 'rgba(0, 0, 0, 0.6)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
    <div onClick={(e) => e.stopPropagation()} style={{ ...glassElevated, padding: 32, maxWidth: 440, width: '90%', boxShadow: theme.shadow.elevated }}>
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
        <button onClick={onCancel} style={{ padding: '10px 22px', borderRadius: theme.radius.md, border: `1px solid ${theme.colors.border.glass}`, background: 'rgba(255,255,255,0.04)', color: theme.colors.text.secondary, fontSize: theme.font.md, fontWeight: 600, cursor: 'pointer', transition: `all ${theme.transition.fast}` }}>
          Abbrechen
        </button>
